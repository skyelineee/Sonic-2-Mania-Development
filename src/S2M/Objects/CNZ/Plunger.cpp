...
void Plunger::Update()
{
    if (this->type == Diagonal) {
        ...
        // BUG: (only sometimes) something here is causing the player to stick to the vertical plunger even after letting go of the jump button, this does not happen for the diagonal type
        // the code is pretty much exactly like it was in v4, so i dont really know whats going wrong
        if (this->plungerActive == true) {
