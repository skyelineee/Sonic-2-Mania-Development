	...
	if (this->timer == 0) {
		for (auto currentPlayer : GameObject::GetEntities<Player>(FOR_ACTIVE_ENTITIES)) {
			if (currentPlayer->CheckCollisionTouch(this, &sVars->hitbox)) {
				if (this->state.Matches(&SlotPrize::State_Winner)) {
					currentPlayer->rings++;
					if (currentPlayer->rings > 999) {
						currentPlayer->rings = 999;
					}

					if (currentPlayer->rings >= currentPlayer->ringExtraLife) { // checks if the ring amount the player has is enough to get a new life
...
void SlotPrize::State_Winner() {}

void SlotPrize::State_Loser() {}
