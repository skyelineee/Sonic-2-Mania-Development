...
void Spiker::Update()
{
    ...
    for (auto currentPlayer : GameObject::GetEntities<Player>(FOR_ACTIVE_ENTITIES)) {
        if (!this->state.Matches(&Spiker::State_Cone)) {
            ...
            if (this->coneState == ConeState::Attached) {
                if (currentPlayer->CheckCollisionTouch(this, &sVars->activateHitbox)) {
                    this->coneState   = (ConeState::Launching);
                    this->launchTimer = 8;
        ...
        if (this->coneState == ConeState::Attached || this->coneState == ConeState::Launching) {
...
void Spiker::Setup()
{
    ...
    this->coneState = (ConeState::Attached);
    this->active    = ACTIVE_BOUNDS;
}
